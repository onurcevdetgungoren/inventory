...
class SatilanPlakalar extends StatefulWidget {
  @override
  _SatilanPlakalarState createState() => _SatilanPlakalarState();
}

//SATIŞ YAPILAN PLAKALARI LİSTELEME.
class _SatilanPlakalarState extends State<SatilanPlakalar> {
  @override
  Widget build(BuildContext context) {
    ...
    return Consumer(
      builder: (context, EbatliViewModel _viewModel, widget) => Scaffold(
        drawer: AppDrawer(),
        appBar: AppBar(
          title: Text("Satılan Plakalar"),
        ),
        body: FutureBuilder(
            future: _viewModel.getSatilanPlakalarView(),
            builder: (context, AsyncSnapshot<List<PlakaSatis>> sonuc) {
              if (sonuc.hasData) {
                print("Gelen:" + sonuc.data.toString());
                return (_viewModel.state == EbatliState.LoadedState)
                    ? ListView.builder(
